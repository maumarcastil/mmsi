...
#v1 = data[data.mmsi == 730000156]
#v1.sort_values(by=["timestamp"], inplace=True, ascending=True)
v1 = data
points = list(v1[["latitude", "longitude"]].to_records(index=False))
data_in_polygon = list()
...
for x in points:
    if(Point(x).within(polygon) == True):
        data_in_polygon.append(x)

poly_data = Polygon(data_in_polygon)


figure = plt.figure()
#plt.plot(*poly_data.exterior.xy)
#plt.scatter(*zip(*data_in_polygon), s=15, c='green')
...
"""
poly_data = Polygon(datos)

figure = plt.figure()
plt.plot(*poly.exterior.xy)
plt.plot(*poly_data.exterior.xy)
plt.scatter(*zip(*datos), s=15, c='green')
plt.scatter(*zip(*coords), s=15, c='goldenrod')
plt.show()
 """
...
#ax.scatter(*zip(*list((i, j) for i, j in zip(v1.latitude, v1.longitude))), c='tab:green', label='tab:green', alpha=0.3) 
